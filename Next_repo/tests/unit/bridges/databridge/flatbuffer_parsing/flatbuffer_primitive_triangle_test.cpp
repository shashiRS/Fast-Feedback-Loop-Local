...
TEST(PRIMITIVE_FBS_CHECK, triangle_fbs_to_json) {
  ...
  std::vector<flatbuffers::Offset<GuiSchema::Triangle>> triangle_vector{};

  auto triangle_basic =
      GuiSchema::CreatePrimitiveBasicDirect(builder, GuiSchema::PrimitiveType::triangle, 5, "#00ff00", 0.9);
  auto triangle_elm = GuiSchema::CreateTriangle(builder, triangle_basic, &center_points, &scales, &rotations);
  triangle_vector.push_back(triangle_elm);

  auto primitve_list_fbs = GuiSchema::CreatePrimitiveListDirect(builder, nullptr, nullptr, &triangle_vector, nullptr,
  ...
  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"].isMember("triangles"),
            true)
      << "reference_input has no data";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"].isMember("triangles"),
            true)
      << "plugin has no data";

  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"].isArray(),
            true)
      << "reference_input output has no valid size";
  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"].size(), 1u)
      << "reference_input output has no valid size";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"].isArray(),
            true)
      << "plugin output has no valid size";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"].size(), 1u)
      << "plugin output has no valid size";

  for (const auto &member_name :
       reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"][0]
           .getMemberNames()) {
    ASSERT_EQ(
        plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"][0].isMember(
            member_name),
        true)
        << "values are not available for " << member_name;
    EXPECT_EQ(
        plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"][0][member_name],
        reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["triangles"][0]
                       [member_name])

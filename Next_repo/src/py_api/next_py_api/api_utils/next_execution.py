...
class NextExecution:
    ...
    def stop_exporter(self, source_name="NextExporter-0", extension_type="", timeout=60):
        """
        Stop the exporter
        Args:
            source_name (str): source name of the exporter.
            extension_type (str): type of output file (.bsig, .csv, .rec)
            timeout (int): Time in seconds to wait for a response
        ...
        comp_name = f"system_SignalExporter::{source_name}"
        if ".rec" in extension_type:
            comp_name = f"system_RecExporter::{source_name}"
        ret_export_stopped, message_exporter = self.status_handler.wait_for_code(
            status_code=status_code_export_stopped, comp_name=comp_name, timeout=timeout, consider_history=True
        )

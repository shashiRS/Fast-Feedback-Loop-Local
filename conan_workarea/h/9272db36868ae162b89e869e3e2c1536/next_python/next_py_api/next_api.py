...
from next_py_api.api_utils import (
    flow_processor,
    next_config_handler,
    next_execution,
...
from next_py_api.api_utils.next_node_process import StartProcessState
from next_py_api.api_utils.status_code_handler import StatusHandler
from next_py_api.api_variables import global_vars

# from api_variables import global_vars
from next_py_api.api_websocket import next_websocket
...
class NextAPI:
        ...
        if not ret:
            for result in result_list:
                if StartProcessState.SUCCESS != result[0]:
                    return global_vars.RUNTIME_ERR
        if backend_config is not None:
    ...
    def load_configuration(self, cfg):
        ...
        :rtype: bool
        """
        return self.config_manager.load_configuration(cfg)
    ...
    # disabling 'too-many-branches' warning from pylint because we cannot reduce the number of it
    ...
    def process_config(self):
        ...
        if not self.load_configuration(self.api_config):
        ...
        for flow_key in flows_to_execute:
            ...
            try:
                ...
                LOG.error(
                    "Failed to get the entry from the configuration. Exception: %s.\n %s", e, traceback.format_exc()
                )
                return global_vars.SETUP_ERR
            ...
            if global_vars.dryrun:
                ...
                if self._previous_backend_settings != flow_entry.backend_settings:
                    if None is not self.status_handler or None is not self.process_handler:
                ...
                is_success = flow_processor.process_flow(
                    run_input, run_export, sim_nodes, sim_runs, self.execute, self.process_handler
                )
                if not is_success:
                    LOG.error("Error during the execution (RUNTIME ERROR)")
                    return global_vars.RUNTIME_ERR
...
def verify_args(args):
    ...
    if args.c is not None:
        if not os.path.exists(args.c):
            LOG.error(
                "Wrong -c option detected: \nLocation (%s) doesn't exist. Please check if config file exists", args.c
            )
            return global_vars.SETUP_ERR
        ...
        if args.i is not None:
            ...
            if not os.path.exists(args.i):
                LOG.error(
                    ...
                    "File (%s) doesn't found. Please enter correct file location",
                    args.i,
                )
                return global_vars.SETUP_ERR
        ...
        if args.f is not None:
            if not isinstance(args.f, str):
                LOG.error(
                    ...
                    "Argument -f (%s) should be a string. Please enter a string",
                    args.f,
                )
                return global_vars.SETUP_ERR
        ...
        if args.rf is not None:
            if not isinstance(args.rf, str):
                LOG.error(
                    ...
                    "Argument -rf (%s) should be a string. Please enter a string",
                    args.rf,
                )
                return global_vars.SETUP_ERR
        ...
        if args.o is not None:
            if not os.path.exists(args.o):
                LOG.error(
                    ...
                    "Location (%s) doesn't exist. Please enter correct base location",
                    args.o,
                )
                return global_vars.SETUP_ERR
...
def main():
    ...
    try:
        ...
        exit_code = verify_args(args)
        if exit_code != global_vars.SUCCESS:
            LOG.error("Failed to verify the arguments from command line")
            sys.exit(exit_code)
        ...
        if not not_found_modules:
            ...
            LOG.error("Following Python module(s) required to run api not found:\n%s", not_found_modules)
            sys.exit(global_vars.SETUP_ERR)
    ...
    # raise run time error
        ...
        LOG.error("-" * 100)
        exit_code = global_vars.RUNTIME_ERR
    ...
    # raise basic exception
        ...
        LOG.error(traceback.format_exc())
        LOG.error("-" * 100)
        exit_code = global_vars.SETUP_ERR

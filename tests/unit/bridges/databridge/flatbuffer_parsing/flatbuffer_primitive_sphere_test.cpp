...
TEST(PRIMITIVE_FBS_CHECK, sphere_fbs_to_json) {
  ...
  std::vector<flatbuffers::Offset<GuiSchema::Sphere>> sphere_vector{};

  auto sphere_basic =
      GuiSchema::CreatePrimitiveBasicDirect(builder, GuiSchema::PrimitiveType::sphere, 5, "#00ff00", 0.9);
  auto sphere_elm = GuiSchema::CreateSphere(builder, sphere_basic, &center_points, &scales, &rotations);
  sphere_vector.push_back(sphere_elm);

  auto primitve_list_fbs = GuiSchema::CreatePrimitiveListDirect(builder, nullptr, nullptr, nullptr, nullptr,
                                                                &sphere_vector, nullptr, nullptr, nullptr, nullptr);
  ...
  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"].isMember("spheres"),
            true)
      << "reference_input has no data";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"].isMember("spheres"), true)
      << "plugin has no data";

  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"].isArray(),
            true)
      << "reference_input output has no valid size";
  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"].size(), 1u)
      << "reference_input output has no valid size";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"].isArray(),
            true)
      << "plugin output has no valid size";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"].size(), 1u)
      << "plugin output has no valid size";

  for (const auto &member_name :
       reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"][0]
           .getMemberNames()) {
    ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"][0].isMember(
                  member_name),
              true)
        << "values are not available for " << member_name;
    EXPECT_EQ(
        plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"][0][member_name],
        reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["spheres"][0][member_name])

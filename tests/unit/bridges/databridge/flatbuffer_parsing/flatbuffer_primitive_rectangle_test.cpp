...
TEST(PRIMITIVE_FBS_CHECK, rectangle_fbs_to_json) {
  ...
  std::vector<flatbuffers::Offset<GuiSchema::Rectangle>> rectangle_vector{};

  auto rect_basic =
      GuiSchema::CreatePrimitiveBasicDirect(builder, GuiSchema::PrimitiveType::rectangle, 5, "#00ff00", 0.9);
  auto park_spot_rect =
      GuiSchema::CreateRectangle(builder, rect_basic, &center_points, &scales, &rotations, false, false);
  rectangle_vector.push_back(park_spot_rect);

  auto primitve_list_fbs = GuiSchema::CreatePrimitiveListDirect(builder, nullptr, &rectangle_vector, nullptr, nullptr,
  ...
  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"].isMember("rectangles"),
            true)
      << "reference_input has no data";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"].isMember("rectangles"),
            true)
      << "plugin has no data";

  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"].isArray(),
            true)
      << "reference_input output has no valid size";
  ASSERT_EQ(reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"].size(),
            1u)
      << "reference_input output has no valid size";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"].isArray(),
            true)
      << "plugin output has no valid size";
  ASSERT_EQ(plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"].size(), 1u)
      << "plugin output has no valid size";

  for (const auto &member_name :
       reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"][0]
           .getMemberNames()) {
    ASSERT_EQ(
        plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"][0].isMember(
            member_name),
        true)
        << "values are not available for " << member_name;
    EXPECT_EQ(
        plugin_output["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"][0][member_name],
        reference_input["dataStream"]["objectData"]["primitives"]["cache_sensor_1"]["data"]["rectangles"][0]
                       [member_name])

...
processor_creator_t create_processor() { return new DummyProcessor(); }
...
std::shared_ptr<next::udex::publisher::PackageProcessor> DummyProcessor::CreateNewInstance() {
  auto dummy = std::make_shared<DummyProcessor>();
...
std::vector<next::sdk::types::DataDescription> DummyProcessor::provideDataDescription() {
...
std::vector<next::sdk::types::IPackage::PackageMetaInfo> DummyProcessor::provideRequestedPackageInfo() {
  ...
  meta.cycle_id = 100;
  meta.virtual_address = 2147483648;
  meta.method_id = 0;
  meta.service_id = 0;
  return {meta};
}
...
DummyProcessor::processPackage(const std::unique_ptr<next::sdk::types::IPackage> &package) {
